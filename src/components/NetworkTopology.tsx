...
export default function NetworkTopology() {
    ...
    const erpNodes: NodeInfo[] = [
        {
            ...
            details: ["Interactions with highly optimized, clean, stateful tabular elements", "Smooth heavy-payload handling bypassing memory leaks of old-gen frames", "Built-in responsive grid layouts customized for diverse enterprise displays"],
            x: 10,
            y: 50,
        },
        {
            ...
            details: ["Multi-tenant domain routing structure directing requests by company namespace", "State-of-the-art token security and single sign-on logic across client systems", "Adaptive rate-limiting protecting mid-tenant services during Peak business hours"],
            x: 35,
            y: 50,
        },
        {
            ...
            details: ["Advanced manufacturing planning matrix algorithms and supply logistics solvers", "Clean domain design separating business transactions from generic read-heavy modules", "Modular, dependency-injected design matching global ERP architecture frameworks"],
            x: 60,
            y: 25,
        },
        {
            ...
            details: ["Pre-calculated data streams representing enterprise profitability indexes", "Extensible JSON schema grids transforming deep recursive accounting sheets in sub-seconds", "Custom interactive controls representing complex ledger states visually"],
            x: 60,
            y: 75,
        },
        {
            ...
            details: ["Heavily tuned physical relational schemas handling millions of production rows per tenant", "Advanced vertical table partitioning preventing transactional blocking on shared catalogs", "Optimized Stored Procedures and CTE indexes returning critical financial metrics under 150ms"],
            x: 90,
            y: 50,
        },
    ];
    ...
    const ragNodes: NodeInfo[] = [
        {
            ...
            details: ["Multi-department workflows routing organizational intent to specific LLM models", "Visual cognitive graphs executing stateful LLM queries based on corporate permissions", "Interactions logs stored securely to meet strict metadata audit specs"],
            x: 10,
            y: 50,
        },
        {
            ...
            details: ["Unified API facade serving consistent payloads across multiple internal models", "Redis-backed prompt caching reducing model operational costs up to 40%", "Load balancing between hot and shadow model instances to guarantee 99.9% uptime"],
            x: 35,
            y: 50,
        },
        {
            ...
            details: ["PagedAttention memory optimization maximizing GPU resource efficiency", "Secured local hardware networks serving sensitive queries within the corporate intranet", "Dynamic request batching handling concurrent corporate chats simultaneously"],
            x: 60,
            y: 20,
        },
        {
            ...
            details: ["Industry-grade security routing external fallback queries over isolated private tunnels", "Context-aware safety filters scrubbing potential PII data before leaving local networks", "High-density token throughput allocation for complex corporate summarization jobs"],
            x: 60,
            y: 80,
        },
        {
            ...
            details: ["Isolated vector collection architectures categorized by security access levels", "Hybrid semantic-keyword searches powered by dense/sparse dual embedding algorithms", "Sub-second query responses across massive knowledge libraries containing thousands of documents"],
            x: 90,
            y: 50,
        },
    ];
    ...
    const getBorderColor = (type: string, isSelected: boolean) => {
        if (isSelected) {
            switch (type) {
                case "client":
                    return "border-indigo-600 bg-indigo-50/70 text-indigo-950 ring-1 ring-indigo-500/20 dark:bg-indigo-900/60 dark:text-indigo-200";
                case "gateway":
                    return "border-sky-600 bg-sky-50/70 text-sky-950 ring-1 ring-sky-500/20";
                case "service":
                    return "border-amber-600 bg-amber-50/70 text-amber-950 ring-1 ring-amber-500/20";
                case "ai":
                    return "border-purple-600 bg-purple-50/70 text-purple-950 ring-1 ring-purple-500/20";
                case "storage":
                    return "border-cyan-600 bg-cyan-50/70 text-cyan-950 ring-1 ring-cyan-500/20";
                default:
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between border-b border-slate-100 bg-slate-50/50 px-6 py-4 gap-4">
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 w-full sm:w-auto">
                    <button
                        ...
                        className={`flex-1 sm:flex-none px-4 py-2 font-display text-xs font-semibold rounded-lg transition-all ${activeTab === "erp" ? "bg-white text-slate-900 shadow-sm border border-slate-200" : "text-slate-500 hover:text-slate-800"}`}
                    >
                        SaaS ERP EverSuite
                    </button>
                    <button
                        ...
                        onClick={() => {
                            setActiveTab("rag");
                            setSelectedNode("milvus");
                        }}
                        className={`flex-1 sm:flex-none px-4 py-2 font-display text-xs font-semibold rounded-lg transition-all ${activeTab === "rag" ? "bg-white text-slate-900 shadow-sm border border-slate-200" : "text-slate-500 hover:text-slate-800"}`}
                    >
                    ...
                    <div className="relative h-[250px] w-full mt-6">
                        {currentNodes.map((node) => (
                            <button
                                ...
                                className={`absolute p-3 rounded-xl border text-left transition-all max-w-[180px] sm:max-w-[200px] z-20 group hover:scale-[1.02] ${getBorderColor(node.type, selectedNode === node.id)}`}
                                style={{
                                    left: `${node.x}%`,
                                    top: `${node.y}%`,
                            ...
                            >
                                <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <span className={`text-[10px] uppercase font-mono tracking-wider font-semibold ${selectedNode === node.id ? "text-slate-800" : "text-slate-400 group-hover:text-slate-650"}`}>{node.type}</span>
                                </div>
                                <div className="text-xs font-semibold leading-tight text-slate-800 group-hover:text-indigo-900 line-clamp-1">{node.label}</div>
                                <div className="text-[10px] text-slate-400 leading-snug font-mono line-clamp-1 mt-0.5">{node.shortDesc}</div>
                            </button>
                        ))}
                    </div>
                ...
                <div className="lg:col-span-4 flex flex-col justify-between bg-white border border-slate-200 rounded-xl p-5 min-h-[380px]">
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            <div className="p-2 bg-slate-50 rounded-lg border border-slate-200">{getNodeIcon(currentNode.type)}</div>
                            <div>
                        ...
                        <div className="mb-4">
                            ...
                            <p className="text-xs bg-slate-50 border border-slate-200 px-3 py-2 rounded-lg font-mono text-indigo-700 leading-relaxed text-[11px]">{currentNode.fullTech}</p>
                        </div>
                    ...
                    <div className="bg-slate-50 border border-slate-200/80 p-3 rounded-lg mt-6">
                        ...
                        <p className="text-[11px] text-slate-500 leading-relaxed font-mono">In production setups, these layers are insulated in virtual private clouds (VPCs) with strict credential policies.</p>
                    </div>
                </div>
            </div>
